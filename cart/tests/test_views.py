...
from cart.services import new_price_and_total_price
...
class CartViewTest(TestCase):
    ...
    def test_view_sent_correct_cart(self):
        self.client.login(email='testuser1', password='TestPass12')
        CartItems.objects.create(user=self.user, product_in_shop=self.goodinmarket, quantity=4, category=self.category)
        self.assertEqual(self.get_response.status_code, 200)
    ...
    def test_change_price_in_cart(self):
        ...
        self.assertEqual(items[0].product_in_shop.seller.title, 'test')
        # new_price_and_total_price(self.client.post('cart:change_price', {'shop': 'test2', 'product_id': 1}, HTTP_X_REQUESTED_WITH='XMLHttpRequest',  HTTP_ACCEPT='application/json'))
        response = self.client.post('cart:change_price', {'product_id': ['1'], 'shop': [' test2']}, HTTP_X_REQUESTED_WITH='XMLHttpRequest', format='json')
        print(response)
        items = CartItems.objects.filter(user=self.user).all()
        print(items[0])
